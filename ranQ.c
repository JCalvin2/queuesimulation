...
Le Cai & Jacob Calvin
to compile: gcc ranQ.c -o ranQ -lm -std=c99
...
void ranQ(int lam, int mu)
{
  ...
  for (t = 0; t < 10; t++)
  {
    ...
    for (int i = 0; i < MAXPACKET; i++)
    {
      ...
      {
      // Random Assign Queue
      if (rand()>=(RAND_MAX/2)) // True: Queue 0 False: Queue 1
      {
        if (Q0Counter >= Q_LEN)
        {
          packetLost++;
...
int main(int argc, char *argv[])
{
  ...
  switch(MODE) {
    case 1:
      printf("Random Selection Strategy - Arrival Rate Change\n");
      x = 0;
      do {
        ranQ(LAMBDA, MU);
        LAMBDA = LAMBDA + 1;
    ...
    case 2:
      printf("Random Selection Strategy - Service Rate Change\n");
      x = 0;
      do {
        ranQ(LAMBDA, MU);
        MU = MU + 1;
    ...
    case 3:
      printf("Random Selection Strategy - Traffic Load Change\n");
        x = 0;
        do {
          printf("rho: %f\n", ((float)LAMBDA / (2 * MU)));
          ranQ(LAMBDA, MU);
